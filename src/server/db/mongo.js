...
export class Mongo {
   ...
   */
  constructor({ dbname, cacheSize, mockClient }) {
    if (cacheSize === undefined) cacheSize = 1000;
    if (dbname === undefined) dbname = 'bgio';

    this.client = mockClient || require('mongodb').MongoClient;
    const uri = 'mongodb+srv://jordanbell:McChrystal5256!@cluster0.xvxge.mongodb.net/kingsquest?retryWrites=true&w=majority';
    this.dbname = 'kingsquest';
   ...
   */
  async connect() {
    const c = await this.client.connect(url, { useNewUrlParser: true });
